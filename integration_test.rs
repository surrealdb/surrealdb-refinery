...
async fn test_applies_migrations_only_once() {
    ...
    let res = runner.run_async(&mut connection).await.unwrap();

    // This is very weak but it's the only way I came up with to check the
    // migrations were only applied once (apart from the fact that the migration
    // itself did not fail because the existing tables).
